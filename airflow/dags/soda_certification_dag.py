from datetime import datetime, timedelta
import os
from airflow import DAG
from airflow.operators.bash import BashOperator
from airflow.operators.dummy import DummyOperator
from airflow.sensors.filesystem import FileSensor
from airflow.utils.dates import days_ago

# Absolute project root (Docker container path)
PROJECT_ROOT = "/opt/airflow"

# Common bash prefix to run in project, load env
BASH_PREFIX = f"cd '{PROJECT_ROOT}' && source .env && "

# Default arguments
default_args = {
    'owner': 'data-team',
    'depends_on_past': False,
    'start_date': datetime(2025, 1, 1),
    'email_on_failure': False,
    'email_on_retry': False,
    'retries': 1,
    'retry_delay': timedelta(minutes=5),
}

# Initialize DAG
with DAG(
    dag_id="soda_certification_pipeline",
    default_args=default_args,
    description="Soda Certification Pipeline: Snowflake setup, dbt models, and Soda data quality checks",
    schedule_interval=None,  # Manual trigger only
    catchup=False,
    tags=["soda", "snowflake", "dbt", "data-quality"],
        doc_md="""
        # Soda Certification Pipeline
        
        This DAG orchestrates the complete Soda Certification pipeline:
        
        ## Initialization Tasks (Run Once)
        - **reset_snowflake**: Reset Snowflake database and schemas
        - **setup_snowflake**: Initialize Snowflake with sample data
        
        ## Pipeline Tasks (Run After Initialization)
        - **dbt_run**: Execute dbt models for all layers (RAW → STAGING → MART)
        - **soda_scan_raw**: Data quality checks on RAW layer
        - **soda_scan_staging**: Data quality checks on STAGING layer  
        - **soda_scan_mart**: Data quality checks on MART layer
        - **soda_scan_quality**: Data quality checks on QUALITY layer
        - **cleanup**: Clean up temporary artifacts
        
        ## Usage
        1. Run initialization tasks once to set up Snowflake
        2. Pipeline tasks automatically run after initialization completes
        3. Results are automatically sent to Soda Cloud platform
        
        ## Task Dependencies
        - Initialization tasks run first (reset_snowflake → setup_snowflake)
        - Pipeline tasks run after initialization (dbt_run → soda_scan_*)
        - All tasks run sequentially to ensure proper data flow
        """,
):

    # =============================================================================
    # INITIALIZATION TASKS (Run Once)
    # =============================================================================
    
    init_start = DummyOperator(
        task_id="init_start",
        doc_md="Start of initialization phase - run once to set up Snowflake"
    )
    
    reset_snowflake = BashOperator(
        task_id="reset_snowflake",
        bash_command=BASH_PREFIX + "python3 scripts/setup/reset_snowflake.py --force",
        doc_md="Reset Snowflake database and schemas (run once)",
    )

    setup_snowflake = BashOperator(
        task_id="setup_snowflake", 
        bash_command=BASH_PREFIX + "python3 scripts/setup/setup_snowflake.py",
        doc_md="Initialize Snowflake with sample data (run once)",
    )
    
    init_end = DummyOperator(
        task_id="init_end",
        doc_md="End of initialization phase - Snowflake is ready"
    )

    # =============================================================================
    # PIPELINE TASKS (Run on Demand)
    # =============================================================================
    
    pipeline_start = DummyOperator(
        task_id="pipeline_start",
        doc_md="Start of pipeline phase - run whenever you want to process data"
    )

    dbt_run = BashOperator(
        task_id="dbt_run",
        bash_command=BASH_PREFIX + "cd dbt && dbt run --profiles-dir . && dbt test --profiles-dir . || true",
        doc_md="Execute dbt models for all layers (RAW → STAGING → MART)",
    )

    # Soda scans for each layer
    soda_raw = BashOperator(
        task_id="soda_scan_raw",
        bash_command=BASH_PREFIX + "soda scan -d soda_certification_raw -c soda/configuration/configuration_raw.yml soda/checks/raw || true",
        doc_md="Data quality checks on RAW layer (lenient thresholds)",
    )

    soda_staging = BashOperator(
        task_id="soda_scan_staging",
        bash_command=BASH_PREFIX + "soda scan -d soda_certification_staging -c soda/configuration/configuration_staging.yml soda/checks/staging || true",
        doc_md="Data quality checks on STAGING layer (stricter thresholds)",
    )

    soda_mart = BashOperator(
        task_id="soda_scan_mart",
        bash_command=BASH_PREFIX + "soda scan -d soda_certification_mart -c soda/configuration/configuration_mart.yml soda/checks/mart || true",
        doc_md="Data quality checks on MART layer (strictest thresholds)",
    )

    soda_quality = BashOperator(
        task_id="soda_scan_quality",
        bash_command=BASH_PREFIX + "soda scan -d soda_certification_quality -c soda/configuration/configuration_quality.yml soda/checks/quality || true",
        doc_md="Data quality checks on QUALITY layer (monitoring)",
    )

    cleanup = BashOperator(
        task_id="cleanup_artifacts",
        bash_command=BASH_PREFIX + "rm -rf dbt/target dbt/logs snowflake_connection_test.log && true",
        doc_md="Clean up temporary artifacts and logs",
    )
    
    pipeline_end = DummyOperator(
        task_id="pipeline_end",
        doc_md="End of pipeline phase - all data quality checks completed"
    )

    # =============================================================================
    # TASK DEPENDENCIES
    # =============================================================================
    
    # Initialization flow (run once)
    init_start >> reset_snowflake >> setup_snowflake >> init_end
    
    # Pipeline flow (run on demand) - MUST run after initialization
    init_end >> pipeline_start >> dbt_run >> [soda_raw, soda_staging, soda_mart, soda_quality] >> cleanup >> pipeline_end
    
    # Note: Pipeline tasks will only run after initialization is complete
